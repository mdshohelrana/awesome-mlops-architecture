...
class TradingDatasetCustom(Dataset):
    def __init__(self, root_path, flag='train', 
                 size=None, features='S', target='Close', 
                 data_path='data.csv', scale=True, timeenc=0, freq='15min', 
                 train_only=False):
        # size [seq_len, label_len, pred_len]
        if size is None:
            self.seq_len = 24 * 4 * 4  # Default to 96
            self.label_len = 24 * 4    # Default to 48
            self.pred_len = 24 * 4     # Default to 48
        else:
            self.seq_len = size[0]
            self.label_len = size[1]
            self.pred_len = size[2]
    ...
    def __read_data__(self):
        ...
        # Drop 'Gmtoffset' column
        df_raw = df_raw.drop(columns=['Gmtoffset'])
        df_raw.columns = df_raw.columns.str.strip()

        # Ensure 'Close' (target) column exists
        if self.target not in df_raw.columns:
            raise ValueError(f"'{self.target}' column not found in the dataset")

        # Reorder columns
        cols = ['Open', 'High', 'Low', 'Volume']  # Feature columns
        df_raw = df_raw[['Datetime'] + cols + [self.target]]

        df_raw = df_raw.dropna()

        # Split into train/val/test sets
        num_train = int(len(df_raw) * 0.7) if not self.train_only else len(df_raw)
        ...
        # Handle different feature sets: 'S', 'M', 'MS'
        if self.features == 'M' or self.features == 'MS':
            df_raw = df_raw[['Datetime'] + cols + [self.target]]
            cols_data = df_raw.columns[1:]  # Skip 'Datetime'
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            df_raw = df_raw[['Datetime'] + [self.target]]
            df_data = df_raw[[self.target]]

        # Scaling data based on the training set
        if self.scale:
            train_data = df_data[border1s[0]:border2s[0]]
            self.scaler.fit(train_data.values)
            data = self.scaler.transform(df_data.values)
        ...
        # Handling time features for 'Datetime'
        ...
        self.data_x = data[border1:border2]  # Features and target
        self.data_y = self.data_x[:, -1].reshape(-1, 1)  # Reshape target 'Close' to [samples, 1]
        self.data_stamp = data_stamp
    ...
    def __getitem__(self, index):
        ...
        # Fetch input sequence
        seq_x = self.data_x[s_begin:s_end]  # Shape: [seq_len, num_features]

        # Fetch only the prediction target (seq_y) for pred_len
        seq_y = self.data_y[r_begin + self.label_len:r_end]  # Only the last pred_len part

        # Fetch time encodings
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
